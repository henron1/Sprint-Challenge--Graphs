...
previous_room = player.currentRoom.id #saving room to access when moving to rooms 

# initalize didctionary 
player_map[player.currentRoom.id] = {}
# record exits to players current room
for exit in player.currentRoom.getExits():
    player_map[player.currentRoom.id][exit] = '?'

# excecute logic to traverse map WHILE visted rooms < total map rooms <<-- if so then we need to keep exploring
while len(player_map) < len(roomGraph):
    exits = player_map[player.currentRoom.id] 

    # use DFS to find a room with no unvisited exits
    q = []
    for i in exits:
        if exits[i] == "?":
            q.append(i)
    ...
    # if unexplored rooms exist, search for a room without an unexplored exit
    if len(q) > 0:
        generated_direction = create_direction(q)

        # set previously traveled room as current room, and travel to the next room in queue
        ...
        reverseDirections.append(generated_direction)

        # when currentRoom has NOT ben visited, add non-visited exits to player_map
        if player.currentRoom.id not in player_map:
            room_exits = {}
            for i in player.currentRoom.getExits(): # i is the exit that we're accessing
                room_exits[i] = '?'

            # update player_map with new room exits and link new room to previous rooms graph
            player_map[previous_room][generated_direction] = player.currentRoom.id
            room_exits[opposite_direction(generated_direction)] = previous_room
            player_map[player.currentRoom.id] = room_exits
        else:
            player_map[previous_room][generated_direction] = player.currentRoom.id
    else:
        returnPath = []
        # we travel back through list of previous directions until we get back to a room with unexplored exits
        for i in reverseDirections[:: -1]: # i is the move we're accessing
            player.travel(opposite_direction(i))
            returnPath.append(opposite_direction(i))
            reverseDirections.pop(-1)
